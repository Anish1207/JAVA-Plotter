...
public class Polynomial extends JFrame 
{
		...
		EventQueue.invokeLater(new Runnable()
		{
			public void run()
			{
				...
				try 
				{
					...
					Polynomial frame = new Polynomial(0,0,0,1);
	 ...
	 * Create the frame.
	 */
	public Polynomial(int a,int b,int c,int d) //constructor
	{
		this.a=a;
		this.b=b;
		this.c=c;
		...
		setTitle("Polynomial");								//set frame title
...
//		setmainpanel(mainpanel);
	...
	{  
		...
		{
			x[j]=xbeg-xstart;
			y[j]=a*((float)Math.pow(x[j],3))+b*((float)Math.pow(x[j],2))+c*x[j]+d;
			if (y[j]<900-row)
				xvisible=(int)x[j];
			xbeg+=least_count;
